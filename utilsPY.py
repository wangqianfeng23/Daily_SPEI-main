...
def fit_gamma_para(
    ...
    for i in range(0, 365):
        ...
        #If there is no data, the value is null 如果没有数据，该值为空
        if now.shape[0] == 0:
        ...
        # If all values are the same, the value is null 如果所有值都相同，则该值为空
        ...
        #Calculate the probability of zero 计算零的概率
        ...
        #If 0 exists and the non-zero values are greater than three, the non-zero values are picked out for likelihood estimation.如果0存在且非零值大于3，则挑出非零值进行似然估计。
        #Add a value close to zero at the end to prevent gaps between the data 在末尾添加一个接近于零的值，以防止数据之间的差距
        # Initial parameters were estimated by L-moments 初始参数由L-moments估计
        ...
        #If there is no value of 0, the parameter estimation is performed directly 如果值不为0，则直接进行参数估计
        #Initial parameters were estimated by L-moments 通过l -矩估计初始参数
        elif est == 0:
            now = now[now > 0]
            paras = distr.gam.lmom_fit(now)
            result_alpa[i] = paras['a']
        ...
        #If there are zero values and the number of non-zero values is less than three, the Moments method is used for initial parameter estimation 当存在零值且非零值个数小于3时，采用矩量法进行初始参数估计
        ...
        #If all values are 0, the argument is null 如果所有值都为0，则参数为空
